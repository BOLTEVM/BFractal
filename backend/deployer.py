...
# Official Fractal Release metadata
...
class SubstrateDeployer:
    ...
    def is_wsl_available(self):
        try:
            ...
            if res.returncode == 0 and res.stdout.strip():
                # Check if default is docker-desktop (which is incompatible)
                if "docker-desktop" in res.stdout.lower() and len(res.stdout.splitlines()) == 1:
                    return False, "DOCKER_DESKTOP_ONLY"
                return True, "READY"
    ...
    async def deploy_node(self):
        ...
        logger.info(f"Downloading Node substrate (Linux build)...")
